...
echo -n  ""> resultsC1_$3.csv

for i in "${!weightNums[@]}";
do
  eval "$2"
  echo -n "${weightNums[$i]}, ">>resultsC1_$3.csv
done
echo "">>resultsC1_$3.csv
...
for ((i=1; i<=$1; i++)); do
    for j in "${!weightNums[@]}";do
        result[$j]=$(./test$j)
        echo -n "${result[$j]}, ">>resultsC1_$3.csv
...
echo " " >> resultsC1_$3.csv
for j in "${!weightNums[@]}";do
    echo -n "${average[$j]} ,">> resultsC1_$3.csv
done
echo "Test completed. Results saved in resultsC1_$3.csv"
