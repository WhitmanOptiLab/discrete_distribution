...
echo -n  ""> resultsJustUpdate_$3.csv

for i in "${!weightNums[@]}";
do
  eval "$2"
  echo -n "${weightNums[$i]}, ">>resultsJustUpdate_$3.csv
done
echo "">>resultsJustUpdate_$3.csv
...
for ((i=1; i<=$1; i++)); do
    for j in "${!weightNums[@]}";do
        result[$j]=$(./test$j)
        echo -n "${result[$j]}, ">>resultsJustUpdate_$3.csv
...
echo " " >> resultsJustUpdate_$3.csv
for j in "${!weightNums[@]}";do
    echo -n "${average[$j]}, ">> resultsJustUpdate_$3.csv
done
echo "Test completed. Results saved in resultsJustUpdate_$3.csv"
