...
//#include "../../../lib/incremental_exponential_zero_weight.hpp"
...
int main() {
  ...
  // dense::stochastic::incremental_exponential_zero_weight<> selector(weights.begin(), weights.end());
  ...
  std::cout << "incremental exponential removing time"<<elapsedtime_sec << std::endl;
  ...
  gettimeofday(&end, NULL);
   elapsedtime_sec = double(end.tv_sec - start.tv_sec) + 
    double(end.tv_usec - start.tv_usec)/1000000.0;
  std::cout << "alex version removing time"<<elapsedtime_sec << std::endl;
