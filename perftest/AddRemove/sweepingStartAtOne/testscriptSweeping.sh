...
echo -n  ""> resultsSweeping_$3.csv

for i in "${!weightNums[@]}";
do
  eval "$2"
  echo -n "${weightNums[$i]}, ">>resultsSweeping_$3.csv
done
echo "">>resultsSweeping_$3.csv
...
for ((i=1; i<=$1; i++)); do
    for j in "${!weightNums[@]}";do
        result[$j]=$(./test$j)
        echo -n "${result[$j]}, ">>resultsSweeping_$3.csv
...
echo " " >> resultsSweeping_$3.csv
for j in "${!weightNums[@]}";do
    echo -n "${average[$j]}, ">> resultsSweeping_$3.csv
done
echo "Test completed. Results saved in resultsSweeping_$3.csv"
