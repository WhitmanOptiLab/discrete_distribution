...
echo -n  ""> resultsBackAndForth_$3.csv

for i in "${!weightNums[@]}";
do
  eval "$2"
  echo -n "${weightNums[$i]}, ">>resultsBackAndForth_$3.csv
done
echo "">>resultsBackAndForth_$3.csv
...
for ((i=1; i<=$1; i++)); do
    for j in "${!weightNums[@]}";do
        result[$j]=$(./test$j)
        echo -n "${result[$j]}, ">>resultsBackAndForth_$3.csv
...
echo " " >> resultsBackAndForth_$3.csv
for j in "${!weightNums[@]}";do
    echo -n "${average[$j]}, ">> resultsBackAndForth_$3.csv
done
echo "Test completed. Results saved in resultsBackAndForth_$3.csv"
