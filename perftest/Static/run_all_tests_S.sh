...
'g++ -std=c++20 -I../../lib -O3 "-DWRSLIB=incremental_leaf_sum_tree<double>"             "-DWEIGHTNUM=${weightNums[$i]}" -o test$i uniform_static.cpp'
...
'g++ -std=c++20 -I../../lib -O3 "-DWRSLIB=old_sideways_fenwick_selector<double>" "-DWEIGHTNUM=${weightNums[$i]}" -o test$i uniform_static.cpp'
