...
echo -n  ""> resultsC3_$3.csv

for i in "${!weightNums[@]}";
do
  eval "$2"
  echo -n "${weightNums[$i]}, ">>resultsC3_$3.csv
done
echo "">>resultsC3_$3.csv
...
for ((i=1; i<=$1; i++)); do
    for j in "${!weightNums[@]}";do
        result[$j]=$(./test$j)
        echo -n "${result[$j]}, ">>resultsC3_$3.csv
...
echo " " >> resultsC3_$3.csv
for j in "${!weightNums[@]}";do
    echo -n "${average[$j]}, ">> resultsC3_$3.csv
done
echo "Test completed. Results saved in resultsC3_$3.csv"
