...
echo -n  ""> resultsC5_$3.csv

for i in "${!weightNums[@]}";
do
  eval "$2"
  echo -n "${weightNums[$i]}, ">>resultsC5_$3.csv
done
echo "">>resultsC5_$3.csv
...
for ((i=1; i<=$1; i++)); do
    for j in "${!weightNums[@]}";do
        result[$j]=$(./test$j)
        echo -n "${result[$j]}, ">>resultsC5_$3.csv
...
echo " " >> resultsC5_$3.csv
for j in "${!weightNums[@]}";do
    echo -n "${average[$j]} ,">> resultsC5_$3.csv
done
echo "Test completed. Results saved in resultsC5_$3.csv"
