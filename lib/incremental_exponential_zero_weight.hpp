#ifndef INCREMENTAL_EXPONENTIAL_ZERO_WEIGHT_LEAF_SUM_TREE 
#define INCREMENTAL_EXPONENTIAL_ZERO_WEIGHT_LEAF_SUM_TREE
#include <vector>
#include <limits>
#include <random>
#include <cassert>
...
class kary_layered_complete_tree_zero_weight {
public:
    ...
    kary_layered_complete_tree_zero_weight(std::vector<Real> leaves){
...
>
class incremental_exponential_zero_weight : protected kary_layered_complete_tree_zero_weight<int_type, Real, fanout> {
  using This = incremental_exponential_zero_weight<int_type, Real, fanout, precision>;
  using BaseTree = kary_layered_complete_tree_zero_weight<int_type, Real, fanout>;
...
public:
  ...
  private:
    ...
    friend class incremental_exponential_zero_weight<int_type, Real, fanout, precision>;
  };

  // Default constructor
  incremental_exponential_zero_weight() : BaseTree(1){}

  // Construct from vector
  incremental_exponential_zero_weight(const std::vector<Real>& weights)
    : BaseTree(weights) {}

  // Construct from initializer list
  incremental_exponential_zero_weight(const std::initializer_list<Real>& il)
    : incremental_exponential_zero_weight(il.begin(), il.end()) {}

  // Construct from iterator pair
  template<class InputIt>
  incremental_exponential_zero_weight(InputIt first, InputIt last)
  ...
  result_type operator()(URNG& g, const Param& param) const {
    incremental_exponential_zero_weight temp(param.weights_);
    return temp(g);
  }
  ...
  void param(const Param& p) {
    *this = incremental_exponential_zero_weight(p.weights_);
  }
