...
class DynamicProposalArrayAlex {
public:
    DynamicProposalArrayAlex(const std::vector<double>& weights) :
    ...
    DynamicProposalArrayAlex(InputIt first, InputIt last) :
    weights_(first, last), R_(std::distance(first, last)), real_dist_(0, 1) {
        assert(weights_.size() > 0);
        N_ = weights_.size();
        W_ = std::accumulate(weights_.begin(), weights_.end(), 0.0);
        avg_ = W_ / N_;
        P_.reserve(2 * N_);
        construct();
    }
    ...
    void update_weight(size_t i, double w) {
        assert(i <= N_);
    ...
    double get_weight(size_t i) const {
        return weights_[i];
    }
