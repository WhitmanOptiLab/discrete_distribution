#ifndef LEAF_SUM_BAD_BRANCHING_TREE
#define LEAF_SUM_BAD_BRANCHING_TREE
#include <vector>
#include <limits>
#include <random>
#include <cassert>
...
class kary_complete_tree_bad_branching {
public:
    ...
    kary_complete_tree_bad_branching() : data_(1, Real(0)), leaf_start_(0), leaf_count_(0) {}

    explicit kary_complete_tree_bad_branching(size_t leaf_count) {
        resize(leaf_count);
    }
...
>
class leaf_sum_tree_bad_branching : protected kary_complete_tree_bad_branching<int_type, Real, fanout> {
  using This = leaf_sum_tree_bad_branching<int_type, Real, fanout, precision>;
  using BaseTree = kary_complete_tree_bad_branching<int_type, Real, fanout>;
...
public:
  ...
  private:
    ...
    friend class leaf_sum_tree_bad_branching<int_type, Real, fanout, precision>;
  };

  // Default constructor
  leaf_sum_tree_bad_branching() : BaseTree(1), leaf_end_(0), leaf_start_(0) {}

  // Construct from vector
  explicit leaf_sum_tree_bad_branching(const std::vector<Real>& weights)
    : leaf_sum_tree_bad_branching(weights.begin(), weights.end()) {}

  // Construct from initializer list
  leaf_sum_tree_bad_branching(const std::initializer_list<Real>& il)
    : leaf_sum_tree_bad_branching(il.begin(), il.end()) {}

  // Construct from iterator pair
  template<class InputIt>
  leaf_sum_tree_bad_branching(InputIt first, InputIt last)
    : BaseTree()
  {
...
result_type operator()(URNG& g) const {
  ...
  result_type operator()(URNG& g, const Param& param) const {
    leaf_sum_tree_bad_branching temp(param.weights_);
    return temp(g);
  }
  ...
  void param(const Param& p) {
    *this = leaf_sum_tree_bad_branching(p.weights_);
  }
