#ifndef INCREMENTAL_EXPONENTIAL_NO_RESIZE 
#define INCREMENTAL_EXPONENTIAL_NO_RESIZE 
#include <vector>
#include <limits>
#include <random>
#include <cassert>
...
class kary_layered_complete_tree_no_resize {
public:
    ...
    kary_layered_complete_tree_no_resize(std::vector<Real> leaves){
      //std::cout<<"kary layered complete tree constructor begun"<<std::endl;
        ...
        resizeVectors();
        num_leafs=layers[0].size();
        num_layers = layers.size();
        calculateLastExpansionSize(num_leafs);

    }

    void calculateLastExpansionSize(size_t numLeafs){
      //since i can't find a logarithm function that takes an input of base in the c++ standard library, I am using the log rule log base a of b = log base c of b / log base c of a
      Real a = std::log2(fanout);
      Real b = std::log2(numLeafs);
      int roundedExponent = b/a;
      lastExpansionSize = std::pow(fanout,roundedExponent)+1;
    }
    ...
    void removeLastNode(){
      // std::cout<<"removing last node. num_layers is "<<num_layers<<std::endl;
      // printTree();
      if (num_leafs==lastExpansionSize){
        num_layers--;
        layers[num_layers][0]=0.0;
        lastExpansionSize= ((lastExpansionSize-1)>>log2_fanout)+1;

        //roundedExponent--;
        //lastExpansionSize = std::pow(fanout,roundedExponent)-1;
      }
      
      position_type i(0,num_leafs-1);
      Real old_weight = value_of(i);
      while(i.first<num_layers){
        value_of(i)-=old_weight;
        parent_of_inplace(i);
      }
      
      num_leafs--;
      // std::cout<<"last none removed      last expansion size = "<<lastExpansionSize<<"           rounded exponenet = "<<roundedExponent<<"       numleafs = "<<num_leafs<<std::endl;
      // printTree();
      return;
    ...
    // void shrinkToNewRoot(){
    //   layers.pop_back();//remove root
    //   layers[0].resize(std::pow(fanout,layers.size()-1));

    //   for(position_type i(0,layers[0].size()-1);i.first<layers.size();parent_of_inplace(i)){
    //     layers[i.first].resize(i.second +1);
    //   }
    // }
    ...
    void addNode(Real weight){
      ...
      int layer=0;
      int index=num_leafs;
      if (num_leafs==layers[0].size()){
        //std::cout<<"num_leafs=layers[0].size(). num leafs is "<<num_leafs<<" and layers[0].size() is "<<layers[0].size()<<std::endl;
      ...
      while (layer<num_layers){
        ...
        layer++;
        if((layer==num_layers)&&(index!=0)){
          layers[num_layers][0]+=layers[num_layers-1][0];
          // std::cout<<"expansion that doesn't push back a new layer";
          // printTree();
          num_layers++;
          lastExpansionSize=num_leafs+1;
      ...
      // std::cout<<"node of weight "<<weight<<" added.    lastExpansionSize = "<<lastExpansionSize<<"       rounded exponent = "<<roundedExponent<<"      numLeafs = "<<num_leafs<<" ";
      // printTree();
      //printTree();
      num_leafs++;
    }
    ...
    void printTree() const{
      int layersToPrint = get_num_layers();
      if (get_num_layers()==layers.size()){
        layersToPrint--;
      }

      std::cout<< "printing tree   num_layers is "<<num_layers<<"     last expansion size is"<<lastExpansionSize<<"          num leafs is"<<num_leafs<< std::endl;
      for(int i=layersToPrint;i>=0;i--){
        std::cout<<"layer "<<i<<": ";
    ...
    position_type root() const {
        return std::pair<int_type,int_type>(num_layers-1,0);
    }
...
private:
    ...
    size_t num_leafs=0;
    size_t num_layers=0;
    size_t lastExpansionSize=0;
    int roundedExponent=0;
    size_t node_count=0;
...
>
class incremental_exponential_no_resize : protected kary_layered_complete_tree_no_resize<int_type, Real, fanout> {
  using This = incremental_exponential_no_resize<int_type, Real, fanout, precision>;
  using BaseTree = kary_layered_complete_tree_no_resize<int_type, Real, fanout>;
...
public:
  ...
  private:
    ...
    friend class incremental_exponential_no_resize<int_type, Real, fanout, precision>;
  };

  // Default constructor
  incremental_exponential_no_resize() : BaseTree(1){}

  // Construct from vector
  incremental_exponential_no_resize(const std::vector<Real>& weights)
    : BaseTree(weights) {}

  // Construct from initializer list
  incremental_exponential_no_resize(const std::initializer_list<Real>& il)
    : incremental_exponential_no_resize(il.begin(), il.end()) {}

  // Construct from iterator pair
  template<class InputIt>
  incremental_exponential_no_resize(InputIt first, InputIt last)
  ...
  result_type operator()(URNG& g, const Param& param) const {
    incremental_exponential_no_resize temp(param.weights_);
    return temp(g);
  }
  ...
  void param(const Param& p) {
    *this = incremental_exponential_no_resize(p.weights_);
  }
  ...
  void update_weight(int_type leafIndex, Real new_weight) {
    ...
    while (i.first < BaseTree::get_num_layers()-1) {
