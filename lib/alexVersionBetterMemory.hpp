#ifndef alex_version_better_memory_BETTER_MEMORY_USE
#define alex_version_better_memory_BETTER_MEMORY_USE
#include <vector>
#include <limits>
#include <random>
#include <cassert>
...
class kary_complete_tree_better_memory {
public:
  ...
  kary_complete_tree_better_memory() : data_(1, Real(0)), leaf_start_(0), leaf_count_(0) {}

  explicit kary_complete_tree_better_memory(size_t leaf_count) {
      resize(leaf_count);
  }
...
>
class alex_version_better_memory : protected kary_complete_tree_better_memory<int_type, Real, fanout> {
  using This = alex_version_better_memory<int_type, Real, fanout, precision>;
  using BaseTree = kary_complete_tree_better_memory<int_type, Real, fanout>;
...
public:
  ...
  private:
    ...
    friend class alex_version_better_memory<int_type, Real, fanout, precision>;
  };

  // Default constructor
  alex_version_better_memory() : BaseTree(1), leaf_end_(0), leaf_start_(0) {}

  // Construct from vector
  explicit alex_version_better_memory(const std::vector<Real>& weights)
    : alex_version_better_memory(weights.begin(), weights.end()) {}

  // Construct from initializer list
  alex_version_better_memory(const std::initializer_list<Real>& il)
    : alex_version_better_memory(il.begin(), il.end()) {}

  // Construct from iterator pair
  template<class InputIt>
  alex_version_better_memory(InputIt first, InputIt last)
    : BaseTree()
  {
...
result_type operator()(URNG& g) const {
  ...
  result_type operator()(URNG& g, const Param& param) const {
    alex_version_better_memory temp(param.weights_);
    return temp(g);
  }
  ...
  void param(const Param& p) {
    *this = alex_version_better_memory(p.weights_);
  }
