#ifndef ZERO_INDEXED_EXPONENTIAL_LEAF_SUM_TREE
#define ZERO_INDEXED_EXPONENTIAL_LEAF_SUM_TREE
#include <vector>
#include <limits>
#include <random>
#include <cassert>
...
class zero_indexed_kary_complete_tree {
public:
  ...
  std::pair<size_t, size_t> minimal_tree_shape(size_t n) {
    ...
    return {total_nodes, leaf_start}; //ZI - one more node in tree so total nodes increases and index of first leaf increases
  }
  ...
  zero_indexed_kary_complete_tree() : data_(1, Real(0)), leaf_start_(0), leaf_count_(0) {}

  explicit zero_indexed_kary_complete_tree(size_t leaf_count) {
      resize(leaf_count);
  }
...
>
class zero_indexed_exponential_leaf_sum_tree : protected zero_indexed_kary_complete_tree<int_type, Real, fanout> {
  using This = zero_indexed_exponential_leaf_sum_tree<int_type, Real, fanout, precision>;
  using BaseTree = zero_indexed_kary_complete_tree<int_type, Real, fanout>;
...
public:
  ...
  private:
    ...
    friend class zero_indexed_exponential_leaf_sum_tree<int_type, Real, fanout, precision>;
  };

  // Default constructor
  zero_indexed_exponential_leaf_sum_tree() : BaseTree(1), leaf_end_(0), leaf_start_(0), max_leaf_(fanout) {}

  // Construct from vector
  explicit zero_indexed_exponential_leaf_sum_tree(const std::vector<Real>& weights)
    : zero_indexed_exponential_leaf_sum_tree(weights.begin(), weights.end()) {}

  // Construct from initializer list
  zero_indexed_exponential_leaf_sum_tree(const std::initializer_list<Real>& il)
    : zero_indexed_exponential_leaf_sum_tree(il.begin(), il.end()) {}

  // Construct from iterator pair
  template<class InputIt>
  zero_indexed_exponential_leaf_sum_tree(InputIt first, InputIt last)
    : BaseTree()
  {
    ...
    for (ptrdiff_t i = leaf_start_ - 1; i >= 0; --i) {
        Real sum = 0;
        PosType first_child = BaseTree::first_child_of(i); // -1-index adjustment
    ...
    total_weight_ = weightsum_of(BaseTree::root());
}
...
result_type operator()(URNG& g) const {
    ...
    PosType node = 1; //first non root node
  ...
  result_type operator()(URNG& g, const Param& param) const {
    zero_indexed_exponential_leaf_sum_tree temp(param.weights_);
    return temp(g);
  }
  ...
  void param(const Param& p) {
    *this = zero_indexed_exponential_leaf_sum_tree(p.weights_);
  }
